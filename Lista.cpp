...
bool Lista_Inserir_Ordenado_Sem_Repeticao(std::string Elemento) {
    ...
    // p/ ignorar as letras maiusculas ou minusculas

    TipoCelula *Temp = Frente;  //Temp aponta p frente

    //enquanto temp for diferente de null faça:
    while (Temp != NULL) { //vai verificar se existem elementos iguais
    ...
    TipoCelula *Novo = (TipoCelula*)malloc(sizeof(TipoCelula)); //cria uma nova celula
    if (Novo == NULL){
        return false;
    }
    ...
    //agora sera ordenado o elemento no lugar correto
    if (Lista_Vazia() || toLower(Frente->Item) > Elemento) {
        ...
        //Insere o elemento no meio ou no fim
